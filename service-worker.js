...
const CACHE='pill-return-v26-ptpfix';
const SHARE_CACHE='pill-return-shares';
const ASSETS=['./manifest.webmanifest','./icon-192.png','./icon-512.png','./ptp-fix.js'];
...
async function injectFix(request){
  const r=await fetch(request,{cache:'no-store'});
  const ct=r.headers.get('content-type')||'';
  if(!ct.includes('text/html')) return r;
  let text=await r.text();
  if(!text.includes('ptp-fix.js')){
    text=text.replace('</body>','<script src="./ptp-fix.js?v=26"></script></body>');
  }
...
self.addEventListener('fetch',event=>{
    ...
    event.respondWith((async()=>{
      try{
        ...
        for(const [,value] of form.entries()){
          if(value instanceof File && (!value.type || value.type.startsWith('image/'))){file=value;break;}
        }
  ...
  if(event.request.mode==='navigate' || event.request.destination==='document'){
    event.respondWith(
      injectFix(event.request).catch(()=>caches.match('./index.html'))
    );
    return;
  }
